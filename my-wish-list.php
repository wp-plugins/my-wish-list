...
function my_wish_donor_process() {
	if( isset( $_POST['wish_donor_add_update'] ) ) {
		...
		$wishmeta[$_POST['wishindex']]['wishdonorname'] = sanitize_text_field( $_POST['wishdonorname'] );
		$wishmeta[$_POST['wishindex']]['wishdonoremail'] = sanitize_email( $_POST['wishdonoremail'] );

		$_POST['wish_donor_thank_you'] = 1;
		$_POST['wish_donor_donation'] = sanitize_text_field( $wishmeta[$_POST['wishindex']]['wishitem'] );
		...
		wp_mail($admin_email, 'Wishlist Item Promised', 'An item ('.sanitize_text_field( $_POST['wishitem'] ).') from your wishlist has been promised by '.sanitize_text_field( $_POST['wishdonorname'] ).'('.sanitize_email( $_POST['wishdonoremail'] ).')', $headers);
	}
}
...
function my_wish_show( $atts ) {
	...
	$wishes = get_post_meta( $id,'wishes',true );

	//thank you message for donors
	if( isset( $_POST['wish_donor_thank_you'] ) ) {
		$output .= '<div class="wishlist-thanks"> Thank you for pledging to get '.sanitize_text_field($_POST['wish_donor_donation']).' for us!  ';
